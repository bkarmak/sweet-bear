...
#include "FoxitEMBSDK_EMBJavaSupport.h"
...
JNIEXPORT int JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFPageStartParse
...
JNIEXPORT int JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFPageContinueParse
  ...
  (JNIEnv *pEnv, jclass cls, jint dib)
{
	...
	LOGI("stride = %x",stride);
	int height = FS_Bitmap_GetHeight(bitmap);
	LOGI("height = %x",height);
	...
	jbyteArray pArray = pEnv->NewByteArray(size);
	pEnv->SetByteArrayRegion(pArray, 0, size, (jbyte*)buffer);
  ...
  (JNIEnv *pEnv, jclass cls, jstring filepath)
{
	...
	const char* file_path;
	file_path = pEnv->GetStringUTFChars(filepath, NULL);
	LOGI("Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: file_path = %s",file_path);
	void* file = fopen(file_path, "wb");
	LOGI("Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: file = %x",file);
	
	void* fileTest = fopen("/data/data/com.foxitsample.annotations/FoxitTest.txt","wb+");
	LOGI("Java_FoxitEMBSDK_EMBJavaSupport_FSFileWriteAlloc: fileTest = %x",fileTest);
...
JNIEXPORT int JNICALL Java_FoxitEMBSDK_EMBJavaSupport_FPDFSignatureAdd
