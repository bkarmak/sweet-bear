...
import com.yangyang.foxitsdk.exception.errorException;
import com.yangyang.foxitsdk.exception.fileAccessException;
import com.yangyang.foxitsdk.exception.formatException;
...
import com.yangyang.foxitsdk.exception.passwordException;
import com.yangyang.foxitsdk.exception.statusException;
import com.yangyang.foxitsdk.exception.toBeContinuedException;
...
public class EMBJavaSupport {
	...
	public class CPDFFormFillerInfo {
		...
		public CPDFFormFillerInfo(IPDFView view) {
			mainView = view;
		}
		...
		public void FFI_Invalidate(int page, float left, float top,
			...
			if (mainView != null) {
				int nCurPage = mainView.getCurrentPageHandler();
				if (nCurPage != page)
					return;
		...
		public int FFI_GetPage(int document, int page_index) {
			if (mainView != null) {
				int nPageHandler = mainView.getPageHandler(page_index);
				return nPageHandler;
			}
			return 0;
		}
		...
		public int FFI_GetCurrentPage(int document) {
			if (mainView != null) {
				return mainView.getCurrentPageHandler();
			}
			return 0;
		}
	...
	public static native int FPSIInitEnvironment(int nCallback,
			boolean bSimulate);

	public static native void FPSIDestroyEnvironment(int nPSIHandler);

	public static native int FPSIInitCanvas(int nPSIHandler, int width,
			int height);
	...
	public static native int FPSIAddPoint(int nPSIHandler, float x, float y,
			float pressure, int flag);

	public static native int FPSIRender(int nPSIHandler, int bitmap, int left,
			int top, int right, int bottom, int src_left, int src_top);

	// public static native int FPSIRenderWithBitmap(int nPSIHandle, Bitmap
	// bitmap, int left, int top, int right, int bottom);
	public static native int FPDFPage_GeneratePSIAnnot(int nPSIHandler,
			int page, Rectangle pageRect, float left, float top, float right,
			float bottom);
